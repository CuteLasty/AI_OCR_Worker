...
class TG4_EMEA_Retail_Label(BaseLabelAnalyzer):
    ...
    def analyze(self, ocr_results, image_width, image_height):
        ...
        try:
            ...
            prefixes = ('AT,', 'AT ,', 'LU,', 'LU ,')
            processed_ocr_results = []
            for ocr_result in ocr_results:
                new_result = ocr_result.copy()
                if new_result['text'].startswith(prefixes):
                    new_result['text'] = new_result['text'].replace(' ', '')
                processed_ocr_results.append(new_result)

            # 處理忽略區域
            ignore_x = int(image_width*0.45)
            ignore_y = 0
            # ignore_y = int(image_height*0.)
            # ocr_results_copy = process_ignore_str_type1(
            #    ocr_results, ignore_x, ignore_y, 0)
            ocr_results_copy = process_region_str(
                processed_ocr_results, ignore_string, ignore_x, ignore_y, Direction.LOWER_RIGHT, 0, analysis)
