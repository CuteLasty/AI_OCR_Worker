...
def draw_ocr_results(image_path, ocr_results, output_path):
    ...
    try:
        ...
        for item in ocr_results:
            ...
            # 將位置轉換為numpy點數組
            points = np.array([[int(p[0]), int(p[1])] for p in positions], dtype=np.int32)
...
def draw_analysis_results(image_path, analysis_results, output_path):
    ...
    try:
        ...
        # 在頂部繪製狀態
        cv2.putText(result, status_text, (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, color, 3)
        ...
        for idx, res in enumerate(analysis_results.get('results', [])):
            ...
            color = (0, 255, 0) if res_match else (0, 0, 255)
            
            # 顯示結果信息
            info_text = f"{status} 規則 {idx+1}: 發現 '{text}'"
            cv2.putText(result, info_text, (20, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
            y_offset += 30
            
            # 顯示預期文字
            expected_text = f"    預期: '{rule_text}'"
            cv2.putText(result, expected_text, (20, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 1)
            y_offset += 40
