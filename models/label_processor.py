...
class LabelProcessor:
    def __init__(self,
                 yolo_label_model_path,
                 yolo_logo_model_path,
                 backup_base="backups",
                 result_base="results",
        ...
        try:
            ...
            # 初始化YOLO
            logger.info("初始化YOLO...")
            self.yolo = YoloModel(yolo_label_model_path)
            self.yolo_obj = YoloModel(
                model_path=yolo_logo_model_path)
    ...
    def process(self, data):
        ...
        try:
            ...
            # 2-1 處理Logo
            detections, pp_raw_image_path = self._detect_and_process_logo(
                raw_image_path)

            # 3. 預處理圖像
            pp_image_path = self._preprocess_image(
                pp_raw_image_path, timestamp)
    ...
    def _detect_and_process_logo(self, image_path):
        """
        用YOLO檢測並清理Logo
        ...
        """
        # 運行YOLO檢測
        detections = self.yolo_obj.detect(image_path)

        EMEA_Logos = ("Paper_Logo", "PAP21_Logo", "UKCA_Logo",
                      "WEEE_Logo", "EU_CE_Logo", "Tidyman_Logo", "FR_Recycle_Logo")
        IN_Logos = ("IN_Recycle_Logo", "IN_BIS_Logo")
        JP_Logo = ("JP_PAP_Logo")
        US_TMO_Logo = ("MX_IFT_Logo")

        pp_raw_image = cv2.imread(image_path)

        logo_list = []
        for det in detections:
            x1, y1, x2, y2 = [int(val) for val in det['xyxy']]
            pp_raw_image[y1:y2, x1:x2] = [255, 255, 255]
            logo_list.append(det['class_name'])

        # 準備目標路徑
        dir_name = os.path.dirname(image_path)
        file_name = os.path.basename(image_path)
        pp_raw_image_path = os.path.join(dir_name, f'pp_{file_name}')

        cv2.imwrite(pp_raw_image_path, pp_raw_image)

        return detections, pp_raw_image_path
