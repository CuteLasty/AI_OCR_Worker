...
from utils.image_utils import crop_image, preprocess_image, draw_ocr_results, draw_analysis_results
...
class LabelProcessor:
    ...
    def process(self, data):
        ...
        try:
            ...
            # 2. YOLO檢測並切割
            largest_box, raw_image_path = self._detect_and_crop(image_path, timestamp)
            ...
            # 5. 保存OCR結果並可視化
            ocr_results_path, ocr_image_path = self._save_ocr_results(ocr_results, raw_image_path, timestamp)
            ...
            # 7. 可視化分析結果
            analysis_image_path = self._visualize_analysis(analysis_results, raw_image_path, timestamp)
    ...
    def _validate_input(self, data):
        ...
        # 檢查時間戳格式
        timestamp = data['timestamp']
        if '_' not in timestamp or len(timestamp) != 15:
            raise ValueError(f"時間戳格式無效，應為'YYYYMMDD_HHMMSS': {timestamp}")
    ...
    def _save_ocr_results(self, ocr_results, raw_image_path, timestamp):
        ...
        result_dir = os.path.dirname(raw_image_path)
        image_name = Path(raw_image_path).stem.replace('raw_', '')
        ocr_results_path = os.path.join(result_dir, "ocr_results.json")
    ...
    def _get_template(self, template_type, sku_name):
        ...
        """
        # 構建模板路徑
        template_path = os.path.join(self.template_dir, template_type, f"{sku_name}.json")
    ...
    def _analyze_barcode(self, ocr_results, data):
        ...
        if ocr_results:
            # 可以根據特定規則或格式來查找SKU名稱
            # 這裡簡化為使用第一個OCR結果
            sku_name = ocr_results[0]['text']
            
        if not sku_name:
            return {
                'match': False,
                'errors': ["未能從OCR結果中確定SKU名稱"]
            }
    ...
    def _analyze_text(self, ocr_results, data):
        ...
        # 使用文字分析器分析
        return self.text_analyzer.analyze(ocr_results, template)
    ...
    def _visualize_analysis(self, analysis_results, raw_image_path, timestamp):
        ...
        # 保存分析結果
        analysis_results_path = os.path.join(result_dir, "analysis_results.json")
